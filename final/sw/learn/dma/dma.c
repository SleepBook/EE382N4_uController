...
//this is OCM
//#define SRC_ADDR 0xfffc0000
//this is BRAM
//#define SRC_ADDR 0x40000000
//this is top of DDR
//#define SRC_ADDR 0x1fffd000
//another mapping for ocm
#define SRC_ADDR 0x10000000
//#define DST_ADDR 0x40001000
#define DST_ADDR 0x1ffff000
#define CP_LEN 128
...
int main() {
    ...
    unsigned int* virtual_address = mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, DMA_ADDR); // Memory map AXI Lite register block
    unsigned int* virtual_source_address  = mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, SRC_ADDR); // Memory map source address
    unsigned int* virtual_destination_address = mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, DST_ADDR); // Memory map destination address

    virtual_source_address[0]= 0x10000000; // Write random stuff to source block
    virtual_source_address[1]= 0x02000000; // Write random stuff to source block
    virtual_source_address[2]= 0x00300000; // Write random stuff to source block
    virtual_source_address[3]= 0x00040000; // Write random stuff to source block
    virtual_source_address[4]= 0x00005000; // Write random stuff to source block

    virtual_source_address[5]= 0xdeadbeaf; // Write random stuff to source block
    ...
    dma_set(virtual_address, S2MM_DESTINATION_ADDRESS, DST_ADDR); // Write destination address
    printf("this is the status after destination address\n");
    dma_s2mm_status(virtual_address);

    //printf("Writing source address...\n");
    dma_set(virtual_address, MM2S_START_ADDRESS, SRC_ADDR); // Write source address
