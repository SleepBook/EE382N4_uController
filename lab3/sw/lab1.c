...
int main(int argc, char *argv[])
{
    ...
    while(1){
        ...
        if(count == TEST_LENGTH){
            for(j=0;j<INTERV_MASK;j++){
                    for(i=0;i<TEST_LENGTH;i++){ //checking starts from 0
                        if(status[i].inter == 0) continue;
                            ...
                            data = *addr;
                            if(rp){
                                printf("Reading to Port B\n");
                                //fprintf(log, "Reading to Port B\n");
                                p_addr = i + PORT_B_ADDR;
                               }
                            else{
                                printf("Reading to Port A\n");
                                //fprintf(log, "Reading to Port A\n");
                                p_addr = i + PORT_A_ADDR;
                            }

                            printf("Readback data from offset 0x%.8x, the value is %u\n", p_addr, data);
                            ...
                            if(data != status[i].data){
                                ...
                                printf("Defect Detected at MEM_OFFSET %u\n",i);
            ...
            *addr = data;
            printf("Writing %u to mem offset %u\n", data, 0);
...
#endif
            *addr = data;
            printf("Writing %u to mem offset %p\n", data, addr);
            //fprintf(log, "Writing %u to mem offset %p\n", data, addr);
            if(wp){
                printf("Writing to Port B\n");
            ...
            printf("Writing %u to mem offset 0x%.8x\n", data, p_addr);
